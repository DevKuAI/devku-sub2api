...
该测试直接调用服务层，不包含 HTTP 网络、移动端、并发用户、多企业或多年历史。以上数值属于首次实现基线。后续优化已减少应用进程载入的旧调用明细；数据库仍需解析历史版本，内容元数据也仍按快照读取。本地结果不作为生产 SLA，须按真实数据分布、并发规模和 PRD 阈值另行验收。

后续查询优化的同进程对照和限制见 [查询优化记录](./query-optimization.md)：10,000 条历史调用样本载入事实从 10,015 条降至 15 条，总览 p95 从 41.861 ms 降至 29.258 ms；近期调用样本的 p95 则增加约 6.4 ms，未将该结果概括为所有场景均加速。
