...
export const useAdminComplianceStore = defineStore('adminCompliance', () => {
  ...
  function requireAcknowledgement(partialStatus?: Partial<AdminComplianceStatus>): void {
    status.value = {
      ...
      document_url_zh: partialStatus?.document_url_zh || status.value?.document_url_zh || 'https://github.com/DevKuAI/devku-sub2api/blob/main/docs/legal/admin-compliance.zh.md',
      document_url_en: partialStatus?.document_url_en || status.value?.document_url_en || 'https://github.com/DevKuAI/devku-sub2api/blob/main/docs/legal/admin-compliance.en.md',
