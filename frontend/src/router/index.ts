...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    path: '/admin/bi',
    name: 'AdminBI',
    component: () => import('@/views/admin/BIOperationsView.vue'),
    meta: {
      requiresAuth: true,
      requiresAdmin: true,
      title: 'BI Operations',
      titleKey: 'admin.bi.title',
      descriptionKey: 'admin.bi.description'
    }
  },
  {
...
router.beforeEach(async (to, _from, next) => {
  ...
  if ((to.meta.requiresPayment || to.meta.requiresRiskControl || to.meta.requiresDesktop || to.meta.requiresSubscription) && !appStore.publicSettingsLoaded) {
    try {
      await appStore.fetchPublicSettings()
    } catch (error) {
      console.warn('Failed to load public settings in route guard', error)
