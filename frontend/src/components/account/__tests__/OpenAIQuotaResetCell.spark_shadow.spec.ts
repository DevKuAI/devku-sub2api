...
describe('OpenAIQuotaResetCell subscription API boundary', () => {
  it('uses the supplied API and hides admin credits and referrals', async () => {
    const api = {
      refresh: vi.fn().mockResolvedValue({ fetched_at: 123, cache_persisted: true }),
      reset: vi.fn(),
    }
    const wrapper = mount(OpenAIQuotaResetCell, {
      props: { account: { id: 7, platform: 'openai', type: 'oauth' }, api },
    })
    expect(wrapper.find('[data-testid="codex-credits"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid="referral-count"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid="referral-open"]').exists()).toBe(false)
    await wrapper.findAll('button')[0].trigger('click')
    await flushPromises()
