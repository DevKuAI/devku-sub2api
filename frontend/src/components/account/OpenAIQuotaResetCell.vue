<template>
  <div v-if="visible" class="space-y-1">
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      <OpenAIReferralCell v-if="!api" :account="account" />
    </div>
...
type QuotaResetAccount = Pick<Account, 'id' | 'platform' | 'type'> & Partial<Pick<Account, 'name' | 'extra' | 'parent_account_id'>> & {
  is_shadow?: boolean
...
const readCachedCredits = (account: QuotaResetAccount) => {
...
const handleQuery = async () => {
  ...
  try {
    const result = await (props.api?.refresh ?? refreshOpenAIQuota)(accountID)
...
const confirmReset = async () => {
  ...
  resetWarning.value = null
  try {
    const result: OpenAIQuotaResetResult = await (props.api?.reset ?? resetOpenAIQuota)(accountID)
    if (props.account.id !== accountID) return
