...
describe('user KeysView column settings', () => {
  ...
  it('excludes Desktop-managed keys from bulk editing', async () => {
    listKeys.mockResolvedValue({
      items: [createApiKey(), createApiKey({ id: 2, managed_by: 'desktop' })],
      total: 2, pages: 1,
    })
    const wrapper = await mountView()
    const table = wrapper.findComponent({ name: 'DataTable' })
    ...
    await nextTick()
    await wrapper.get('[data-test="bulk-edit-keys"]').trigger('click')
    expect(wrapper.findComponent({ name: 'BulkEditKeysModal' }).props('selectedKeys').map((key: ApiKey) => key.id)).toEqual([1])
    wrapper.unmount()
  })
