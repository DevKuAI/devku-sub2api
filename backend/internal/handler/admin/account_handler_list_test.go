...
func TestAccountHandlerListLiteUsesCompactDTOAndETag(t *testing.T) {
	...
	adminSvc.accounts = []service.Account{{
		...
		BoundUserID: &boundUserID, BoundUser: &service.AccountBoundUser{ID: boundUserID, Username: "bound-user", Email: "bound@example.com"},
	...
	require.Equal(t, float64(boundUserID), liteItem["bound_user_id"])
	boundUser, ok := liteItem["bound_user"].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "bound@example.com", boundUser["email"])
