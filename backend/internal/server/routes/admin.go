...
func RegisterAdminRoutes(
	...
	registerBIAdminRoutes(admin, h, cfg, stepUpAuth)
	{
		// 部署与运营合规确认
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.POST("/:id/impersonate", h.Auth.ImpersonateUser)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		adminSettings.GET("/bi-privacy", h.Admin.Setting.GetBIPrivacyNotice)
		adminSettings.PUT("/bi-privacy", h.Admin.Setting.UpdateBIPrivacyNotice)
