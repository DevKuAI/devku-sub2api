...
func TestAccountReadableSnapshot_DenylistTripwire(t *testing.T) {
	...
	stripped := map[string]struct{}{
		"Credentials": {}, "Groups": {}, "AccountGroups": {}, "BoundUserID": {}, "BoundUser": {},
	}
	...
	boundUserID := int64(321)
	acct.BoundUserID = &boundUserID
	acct.BoundUser = &AccountBoundUser{ID: boundUserID, Email: "private-bound-user@example.test"}
	...
	assert.Nil(t, m["BoundUserID"])
	assert.Nil(t, m["BoundUser"])
	assert.NotContains(t, string(snap), "private-bound-user@example.test")
	...
	assert.Equal(t, &boundUserID, acct.BoundUserID)
	assert.Equal(t, "private-bound-user@example.test", acct.BoundUser.Email)
