...
type pluginDirectoryAccountRepoStub struct {
	AccountRepository
	accounts []Account
}

func (r *pluginDirectoryAccountRepoStub) ListByPlatform(context.Context, string) ([]Account, error) {
	return r.accounts, nil
}

func TestProvidePluginManager_AccountDirectoryCapability(t *testing.T) {
	repo := &pluginDirectoryAccountRepoStub{accounts: []Account{
		...
		{ID: 9, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusError},
	}}
	...
	authorized := manager.buildHostServices(&PluginInstallation{PluginKey: "p.authorized", Manifest: PluginManifest{
		Capabilities: []PluginCapability{{ID: PluginCapabilityOpenAIOAuthOutbound, Platform: PlatformOpenAI, AccountType: AccountTypeOAuth}},
	}})
	require.NotNil(t, authorized)
	accounts, err := authorized.ListAccounts(ctx, &pluginv1.ListAccountsRequest{Platform: PlatformOpenAI, AccountType: AccountTypeOAuth})
	require.NoError(t, err)
	assert.Equal(t, []int64{7}, accounts.AccountIds)

	unauthorized := manager.buildHostServices(&PluginInstallation{PluginKey: "p.other"})
	require.NotNil(t, unauthorized)
	_, err = unauthorized.ListAccounts(ctx, &pluginv1.ListAccountsRequest{})
	assert.Equal(t, codes.Unavailable, status.Code(err))
	_, err = unauthorized.KVGet(ctx, &pluginv1.KVGetRequest{Namespace: "state", Key: "k"})
	require.NoError(t, err)
}
