...
func TestAnthropicEventToResponses_Opus55RedactedThinkingClosesBeforeText(t *testing.T) {
	state := NewAnthropicEventToResponsesState()
	AnthropicEventToResponsesEvents(&AnthropicStreamEvent{
		Type: "message_start", Message: &AnthropicResponse{ID: "msg_1", Model: "claude-opus-5-5"},
	}, state)
	index := 0
	AnthropicEventToResponsesEvents(&AnthropicStreamEvent{
		Type: "content_block_start", Index: &index,
		ContentBlock: &AnthropicContentBlock{Type: "redacted_thinking", Data: "opaque-thinking"},
	}, state)
	events := AnthropicEventToResponsesEvents(&AnthropicStreamEvent{Type: "content_block_stop", Index: &index}, state)
	var closed *ResponsesOutput
	for _, event := range events {
		if event.Type == "response.output_item.done" {
			closed = event.Item
		}
	}
	if closed == nil || closed.Type != "reasoning" || closed.EncryptedContent == "" || len(closed.Summary) != 0 {
		t.Fatalf("redacted thinking must close as opaque reasoning, got %#v", closed)
	}
	index = 1
	...
	if len(events) != 2 || events[0].Type != "response.output_item.added" || events[0].OutputIndex != 1 || events[1].ContentIndex != 0 {
		t.Fatalf("text must start in the next output item, got %#v", events)
	}
}
