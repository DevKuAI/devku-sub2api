...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	desktopResourceService := service.NewDesktopResourceService(desktopResourceRepository)
	desktopResourceHandler := handler.NewDesktopResourceHandler(desktopResourceService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, handlerDesktopHandler, handlerDesktopUpdateHandler, desktopResourceHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
