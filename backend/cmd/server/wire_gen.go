...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	biHandler := bi.NewHandler(db, redisClient, configConfig, authService, userService, settingService)
	...
	contentModerationService := service.ProvideContentModerationService(settingRepository, contentModerationRepository, contentModerationHashCache, groupRepository, userRepository, proxyRepository, apiKeyAuthCacheInvalidator, emailService, settingService)
	...
	desktopRepository := repository.NewDesktopRepository(client, apiKeyRepository)
	desktopUsageRepository := repository.NewDesktopUsageRepository(client, db)
	desktopRefreshStore := repository.NewDesktopRefreshStore(redisClient)
	desktopLoginLimiter := repository.NewDesktopLoginLimiter(redisClient, configConfig)
	desktopTokenManager, err := service.NewDesktopTokenManager(configConfig)
	if err != nil {
		return nil, err
	}
	...
	desktopUpdateService := service.ProvideDesktopUpdateService(desktopUpdateRepository, desktopUpdateArtifactStorageFactory, configConfig)
	desktopUpdateHandler := admin.NewDesktopUpdateHandler(desktopUpdateService)
	desktopResourceRepository := repository.NewDesktopResourceRepository(db)
	desktopResourceStorage := repository.NewDesktopResourceStorage(configConfig)
	desktopResourceService := service.NewDesktopResourceService(desktopResourceRepository, desktopResourceStorage, configConfig)
	desktopResourceHandler := admin.NewDesktopResourceHandler(desktopResourceService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, desktopHandler, desktopUpdateHandler, desktopResourceHandler, desktopService, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	tiboResetMonitorCache := repository.NewTiboResetMonitorCache(redisClient)
	tiboResetMonitorService := service.NewTiboResetMonitorService(tiboResetMonitorCache)
	tiboResetMonitorHandler := handler.NewTiboResetMonitorHandler(tiboResetMonitorService)
	handlerDesktopHandler := handler.NewDesktopHandler(desktopService)
	handlerDesktopUpdateHandler := handler.NewDesktopUpdateHandler(desktopUpdateService)
	handlerDesktopResourceHandler := handler.NewDesktopResourceHandler(desktopResourceService, desktopService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(biHandler, authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, tiboResetMonitorHandler, handlerDesktopHandler, handlerDesktopUpdateHandler, handlerDesktopResourceHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
