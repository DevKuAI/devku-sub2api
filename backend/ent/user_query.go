...
type UserQuery struct {
	...
	withDesktopOrganizations  *DesktopOrganizationQuery
	withBoundAccounts         *AccountQuery
...
// QueryDesktopOrganizations chains the current query on the "desktop_organizations" edge.
func (_q *UserQuery) QueryDesktopOrganizations() *DesktopOrganizationQuery {
...
// QueryBoundAccounts chains the current query on the "bound_accounts" edge.
func (_q *UserQuery) QueryBoundAccounts() *AccountQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withDesktopOrganizations:  _q.withDesktopOrganizations.Clone(),
		withBoundAccounts:         _q.withBoundAccounts.Clone(),
...
// WithDesktopOrganizations tells the query-builder to eager-load the nodes that are connected to
// the "desktop_organizations" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithDesktopOrganizations(opts ...func(*DesktopOrganizationQuery)) *UserQuery {
	query := (&DesktopOrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDesktopOrganizations = query
	return _q
}

// WithBoundAccounts tells the query-builder to eager-load the nodes that are connected to
// the "bound_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithBoundAccounts(opts ...func(*AccountQuery)) *UserQuery {
	query := (&AccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBoundAccounts = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [16]bool{
			...
			_q.withDesktopOrganizations != nil,
			_q.withBoundAccounts != nil,
	...
	if query := _q.withDesktopOrganizations; query != nil {
		if err := _q.loadDesktopOrganizations(ctx, query, nodes,
			func(n *User) { n.Edges.DesktopOrganizations = []*DesktopOrganization{} },
			func(n *User, e *DesktopOrganization) {
				n.Edges.DesktopOrganizations = append(n.Edges.DesktopOrganizations, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := _q.withBoundAccounts; query != nil {
		if err := _q.loadBoundAccounts(ctx, query, nodes,
			func(n *User) { n.Edges.BoundAccounts = []*Account{} },
			func(n *User, e *Account) { n.Edges.BoundAccounts = append(n.Edges.BoundAccounts, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadDesktopOrganizations(ctx context.Context, query *DesktopOrganizationQuery, nodes []*User, init func(*User), assign func(*User, *DesktopOrganization)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(desktoporganization.FieldGatewayUserID)
	}
	query.Where(predicate.DesktopOrganization(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DesktopOrganizationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "gateway_user_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadBoundAccounts(ctx context.Context, query *AccountQuery, nodes []*User, init func(*User), assign func(*User, *Account)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Account(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BoundAccountsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.BoundUserID
		if fk == nil {
			return fmt.Errorf(`foreign-key "bound_user_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
