...
type Account struct {
	...
	// User allowed to view this subscription account; one user may own multiple accounts.
...
func (*Account) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case account.FieldID, account.FieldProxyID, account.FieldProxyFallbackOriginID, account.FieldBoundUserID, account.FieldConcurrency, account.FieldLoadFactor, account.FieldPriority, account.FieldParentAccountID:
	...
	for i := range columns {
		...
		case account.FieldBoundUserID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.BoundUserID = new(int64)
				*_m.BoundUserID = value.Int64
			}
