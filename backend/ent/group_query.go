...
func (_q *GroupQuery) QueryDesktopOrganizations() *DesktopOrganizationQuery {
...
func (_q *GroupQuery) WithDesktopOrganizations(opts ...func(*DesktopOrganizationQuery)) *GroupQuery {
	query := (&DesktopOrganizationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withDesktopOrganizations; query != nil {
		if err := _q.loadDesktopOrganizations(ctx, query, nodes,
			func(n *Group) { n.Edges.DesktopOrganizations = []*DesktopOrganization{} },
			func(n *Group, e *DesktopOrganization) {
...
func (_q *GroupQuery) loadDesktopOrganizations(ctx context.Context, query *DesktopOrganizationQuery, nodes []*Group, init func(*Group), assign func(*Group, *DesktopOrganization)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(desktoporganization.FieldGroupID)
	}
	query.Where(predicate.DesktopOrganization(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.DesktopOrganizationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
