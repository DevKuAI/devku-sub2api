...
type APIKeyQuery struct {
	...
	withDesktopMemberAssignment *DesktopMemberAPIKeyQuery
...
// QueryDesktopMemberAssignment chains the current query on the "desktop_member_assignment" edge.
func (_q *APIKeyQuery) QueryDesktopMemberAssignment() *DesktopMemberAPIKeyQuery {
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withDesktopMemberAssignment: _q.withDesktopMemberAssignment.Clone(),
...
// WithDesktopMemberAssignment tells the query-builder to eager-load the nodes that are connected to
// the "desktop_member_assignment" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithDesktopMemberAssignment(opts ...func(*DesktopMemberAPIKeyQuery)) *APIKeyQuery {
	query := (&DesktopMemberAPIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDesktopMemberAssignment = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			_q.withDesktopMemberAssignment != nil,
		}
	)
	...
	if query := _q.withDesktopMemberAssignment; query != nil {
		if err := _q.loadDesktopMemberAssignment(ctx, query, nodes, nil,
			func(n *APIKey, e *DesktopMemberAPIKey) { n.Edges.DesktopMemberAssignment = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadDesktopMemberAssignment(ctx context.Context, query *DesktopMemberAPIKeyQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *DesktopMemberAPIKey)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(desktopmemberapikey.FieldAPIKeyID)
	}
	query.Where(predicate.DesktopMemberAPIKey(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.DesktopMemberAssignmentColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
