...
Review these conditions before deploying or using Sub2API:

- **Upstream terms:** Using Sub2API may violate the terms of service of Anthropic or other upstream providers. Review each provider's agreement before use. You are responsible for the resulting risks.
- **Legal compliance:** Use Sub2API only when permitted by the laws and regulations of your country or region. Do not use it for unlawful activity.
- **Disclaimer:** This project is provided only for technical learning and research. The authors are not liable for account suspensions, service interruptions, data loss, or other direct or indirect damages resulting from its use.
- **No commercial authorization:** The developers have not authorized any individual or organization to operate commercial services in the project's name or based on the project. The operator is solely responsible for any resulting disputes, losses, or legal liability.

## Overview

Sub2API is an API gateway for distributing and managing subscription quotas from upstream AI services. It manages user API keys, upstream account access, authentication, usage accounting, billing, request routing, and load balancing.

## Features

- **Upstream account management:** Supports OAuth and API key-based upstream accounts.
- **API key management:** Issues and manages user API keys. Administrators can set the maximum number of keys each user can create.
- **Usage accounting and billing:** Tracks token usage and calculates costs.
- **Request scheduling:** Selects upstream accounts and supports sticky sessions.
- **Concurrency and rate limits:** Applies per-user and per-account concurrency limits, request rate limits, and token rate limits.
- **Built-in payments:** Supports EasyPay, Alipay, WeChat Pay, and Stripe for user self-service top-ups without a separate payment service ([Configuration Guide](docs/PAYMENT.md)).
