...
GITHUB_API_CURL_HELPER="$TEMP_DIR/github-api-curl.sh"
sed -n '/^github_api_curl()/,/^}/p' "$ROOT_DIR/deploy/install.sh" > "$GITHUB_API_CURL_HELPER"
grep -Fq 'github_api_curl()' "$GITHUB_API_CURL_HELPER"
...
run_api_curl() {
    CURL_ARGS_LOG="$1" HOME="$TEMP_DIR/home" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="${2:-}" \
        ...
        "$GITHUB_API_CURL_HELPER" "https://api.github.com/repos/DevKuAI/devku-sub2api/releases/latest"
}
...
test "$(grep -Fxc 'https://api.github.com/repos/DevKuAI/devku-sub2api/releases/latest' "$TEMP_DIR/authenticated")" -eq 1
...
test "$(grep -Fxc 'https://api.github.com/repos/DevKuAI/devku-sub2api/releases/latest' "$TEMP_DIR/anonymous")" -eq 1
...
assert_unsafe_invocation_rejected() {
    ...
    if CURL_ARGS_LOG="$TEMP_DIR/$name" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="update-secret" \
        ...
        "$GITHUB_API_CURL_HELPER" "$@" 2>/dev/null; then
...
assert_unsafe_invocation_rejected non-api -s \
    "https://github.com/DevKuAI/devku-sub2api/releases/download/v1/asset"
assert_unsafe_invocation_rejected mixed-host -s \
    "https://api.github.com/repos/DevKuAI/devku-sub2api/releases/latest" \
    "https://example.com/collect"
assert_unsafe_invocation_rejected multiple-api -s \
    "https://api.github.com/repos/DevKuAI/devku-sub2api/releases/latest" \
    "https://api.github.com/repos/DevKuAI/devku-sub2api/releases"
