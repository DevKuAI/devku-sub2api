...
# Sub2API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/DevKuAI/devku-sub2api/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="DevKuAI/devku-sub2api"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/sub2api" ]; then
        ...
        "$INSTALL_DIR/sub2api" --version 2>&1 | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+(\.[0-9]+)?' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
Documentation=https://github.com/DevKuAI/devku-sub2api
