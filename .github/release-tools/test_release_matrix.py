...
class ReleaseMatrixTest(unittest.TestCase):
    ...
    def fixture_artifacts(self, simple=False):
        ...
        for target in release.targets(simple):
            ...
            metadata = {'version': '9.8.7.0', 'sha': 'a' * 40, 'target': target,
        ...
        return argparse.Namespace(input='release-input', version='9.8.7.0', sha='a' * 40, simple=simple, output='contexts')
    ...
    def test_plan_rejects_upstream_and_invalid_fork_tags(self):
        for tag in ('v9.8.7', 'v9.8.7.01', 'v9.8.7.0-beta', '9.8.7.0'):
            with self.subTest(tag=tag), patch.object(subprocess, 'check_output', return_value='a' * 40 + '\n'):
                with self.assertRaisesRegex(ValueError, 'four-part fork release version tag'):
                    release.plan(argparse.Namespace(ref=tag, dry_run=False, simple=False))
                self.assertEqual(release.VERSION_FILE.read_text(), '9.8.7.0\n')

    def test_plan_accepts_a_matching_fork_tag(self):
        with patch.dict(os.environ, {'GITHUB_OUTPUT': 'outputs'}), patch.object(subprocess, 'check_output', return_value='a' * 40 + '\n'):
            release.plan(argparse.Namespace(ref='v9.8.7.0', dry_run=False, simple=False))
        output = dict(line.split('=', 1) for line in Path('outputs').read_text().splitlines())
        self.assertEqual(output['tag'], 'v9.8.7.0')
        self.assertEqual(output['version'], '9.8.7.0')
    ...
    def test_docker_commands_do_not_publish_during_dry_run(self):
        ...
        env = {**os.environ, 'PATH': str(fake_bin.resolve()) + os.pathsep + os.environ['PATH'],
               ...
               'RELEASE_VERSION': '9.8.7.0', 'RELEASE_SHA': 'a' * 40, 'GITHUB_REPOSITORY': 'ExampleOwner/sub2api',
        ...
        self.assertIn('ghcr.io/exampleowner/devku-sub2api', log)
        ...
        for simple in (False, True):
            with self.subTest(simple=simple):
                ...
                if simple:
                    ...
                    self.assertNotIn('imagetools', log)
                    self.assertIn('ghcr.io/exampleowner/devku-sub2api:latest', log)
                else:
                    self.assertEqual(log.count('imagetools create'), 2)
                    self.assertIn('huanghua581/devku-sub2api:9.8', log)
                    self.assertIn('ghcr.io/exampleowner/devku-sub2api:9', log)
                    self.assertIn('huanghua581/devku-sub2api:9.8.7.0', log)
                    self.assertNotIn('fixturehub/sub2api', log)
