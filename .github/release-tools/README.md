...
New publications require a four-part fork tag (`vX.Y.Z.N`), validated with `backend/scripts/release-version.sh --fork`. Three-part tags remain reserved for upstream. Images are published to `ghcr.io/devkuai/devku-sub2api` and, when Docker Hub credentials are present, `huanghua581/devku-sub2api`.
